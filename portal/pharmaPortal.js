...
// languageDB.addItem(new sap.ui.core.ListItem({text:"Hindi"}));
// languageDB.addItem(new sap.ui.core.ListItem({text:"Tamil"}));
// languageDB.addEventDelegate({onAfterRendering: function(e) {languageDB.$().find("input").attr("readonly", true);}});

var languageDB = new sap.m.ComboBox({
    layoutData: new sap.m.OverflowToolbarLayoutData({priority: "Low"}),
    selectedKey:'0',
    items:[
        new sap.ui.core.ListItem({key:"0", text: oBundle.getText("English")}),
        new sap.ui.core.ListItem({key:"1", text: oBundle.getText("Hindi")}),
        new sap.ui.core.ListItem({key:"2", text: oBundle.getText("Spanish")})   
    ],
    selectionChange:function(){
        var selectedValue = languageDB.getSelectedKey();
        if(selectedValue == "0"){
            languageCode = "en";
            ...
            languageCode = "hi";
            localStorage.setItem('languageCode', "HI");
...
}).addEventDelegate({onAfterRendering: function(e) {languageDB.$().find("input").attr("readonly", true);}});

console.log(languageCode)
if (languageCode.includes("EN")) {
    oBundle = jQuery.sap.resources({ url: "i18n/i18n_en.properties" });
    languageDB.setValue("English");
    languageDB.setSelectedKey("0");
} else if (languageCode.includes("HI")) {
    oBundle = jQuery.sap.resources({ url: "i18n/i18n_hi.properties" });
    languageDB.setValue("Hindi");
    languageDB.setSelectedKey("1");
} else if (languageCode.includes("ES")) {
    oBundle = jQuery.sap.resources({ url: "i18n/i18n_es.properties" });
    languageDB.setSelectedKey("2");
    console.log('Hy')
}  else {
    oBundle = jQuery.sap.resources({ url: "i18n/i18n_en.properties" });
    languageDB.setValue("English");
...
var logout = new sap.m.Button({
    ...
    press: function () {
        sap.m.MessageBox.show(oBundle.getText("Do_you_want_to_logout"),
            ...
            function (bConfirmed) {
                if (bConfirmed == "YES") {
                    window.open("https://peearz.com/portal/pharma.html", "_self");
                }
            })
}});
...
var toolHeader = new sap.tnt.ToolHeader({
    content: [
        ...
        new sap.m.Image({ src: "Images/logo.png", width: "45px", height: "55px",
        layoutData: new sap.m.OverflowToolbarLayoutData({priority: "NeverOverflow"})}).addStyleClass("sapUiTinyMarginTop"), 
        new sap.m.Label({ text: oBundle.getText("Peearz_Pharma_Portal"), design: sap.m.LabelDesign.Bold }),
